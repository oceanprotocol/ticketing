...
export default function useWalletBallace({ provider }: useWalletBalanceProps) {
  ...
  const [networkTokenBalance, setNetworkTokenBalance] = useState(new BigNumber(0));
  const [oceanTokenBalance, setOceanTokenBalance] = useState(new BigNumber(0));

  const networkTokenBalanceFormatted = useMemo(() => formatNumber(networkTokenBalance, 4), [networkTokenBalance]);
